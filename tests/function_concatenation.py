...
class MyTest(unittest.TestCase):
    ...
    def concat_test(self):
        ...
        t_right = 1.0
        t_left = 0.0
        f_nom, f_test = get_random_function((t_left, t_right), sp_var)
        for _ in range(4):
            f_nom_rand, f_test_rand = get_random_function(
                (t_left, t_right), sp_var)

            f_nom = sp.Piecewise(
                (f_nom, sp_var < -1), (f_nom_rand, 1 < sp_var))
