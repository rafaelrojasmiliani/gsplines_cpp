...
install_deps(){
    apt-get update
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o Dpkg::Options::="--force-confnew"  vim vim-gtk3 vim-nox build-essential cmake python3-dev mono-complete golang nodejs default-jdk npm clang-tidy-9 clang-format-10 apt-transport-https ca-certificates gnupg software-properties-common wget g++-8 golang clang
    update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-7 700 --slave /usr/bin/g++ g++ /usr/bin/g++-7
    update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-8 800 --slave /usr/bin/g++ g++ /usr/bin/g++-8
    wget -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | sudo apt-key add -
    apt-add-repository 'deb https://apt.kitware.com/ubuntu/ bionic main'
    sudo apt-get update
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o Dpkg::Options::="--force-confnew"  cmake libxml2-utils clang-format clang-tidy exuberant-ctags
    npm install --save-dev --save-exact prettier
    sudo pip3 install cmakelang autopep8 pylint flake8 prospector yamllint
